...
public class Test {
	...
	public static void main(String[] args) {
		...
		for (i = 0; i < sLength; i++) {
			...
			externalTable[i] = new ExternalTable(i, data);
		}
		System.out.println("\n外存设置成功");
		//请求页表
		do {
			...
			if (TLB.getCurrent() > 0) {
				...
				if (number != -1 && number != -2) {
					result = memoryData[TLB.quickTable[number].getPhysicsNumber()];
			...
			if (TLB.getCurrent() <= 0 || number == -1) {
				...
				if (pageinnerTable.current <= 0 || number == -1) {
					...
					//在页表找不到，去外存区找
					for (i = 0; i < sLength; i++) {
						if (k1 == externalTable[i].getPageNumber())//在外存找到了缺页
						{
							...
							//调入内存
							pageinnerTable.pageTable[t].setPageNumber(k1);
							if (k2 == 1) {
